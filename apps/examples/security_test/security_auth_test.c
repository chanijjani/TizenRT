...
test_authenticate(void)
{
	...
	printf("ok\n");
	printf("  . SEE Get Random ...\n");
	fflush(stdout);
	if (0 != auth_generate_random(100, &rand)) {
	...
	printf("  . SEE Get Certificate ...\n");
	fflush(stdout);
	if (0 != auth_get_certificate(ARTIK_CERT, &cert)) {
	...
	printf("  . SEE Get Hash ...\n");
	fflush(stdout);
	if (0 != auth_get_hash(HASH_SHA256, &plaintext, &hashed)) {
		printf("  fail\n  ! auth_get_hash\n");
		goto exit;
	}
	printf("ok\n");
	...
	printf("  . SEE Get HMAC ...\n");
	fflush(stdout);
	if (0 != auth_get_hmac(HMAC_SHA256, HMACSHA256_KEY, &plaintext, &hmac)) {
		printf("  fail\n  ! auth_get_hmac\n");
		goto exit;
	}
	printf("ok\n");
