...
static audio_manager_result_t find_audio_card_if_empty(audio_io_direction_t direct);
...
static audio_manager_result_t get_supported_sample_rate(audio_io_direction_t direct);
...
static audio_manager_result_t init_audio_stream(audio_io_direction_t direct)
{
	...
	if (find_audio_card_if_empty(direct) != AUDIO_MANAGER_SUCCESS) {
...
static audio_manager_result_t find_audio_card_if_empty(audio_io_direction_t direct)
{
	...
	if (direct == INPUT) {
		if (g_actual_audio_in_card_id == INVALID_ID) {
		...
		if (g_actual_audio_out_card_id == INVALID_ID) {
...
static audio_manager_result_t get_supported_sample_rate(audio_io_direction_t direct)
{
	...
	ret = find_audio_card_if_empty(direct);
	if (ret != AUDIO_MANAGER_SUCCESS) {
		return ret;
	}
	if (direct == INPUT) {
	...
	card->resample.channels = caps_desc.caps.ac_channels;
	medvdbg("samprate_type : %d\n", card->resample.samprate_types);
...
static unsigned int resample_stream_in(audio_card_info_t *card, void *data, unsigned int frames)
{
	...
	while (frames > used_frames) {
		uint8_t channels = card->resample.channels * card->resample.channel_ratio;
		srcData.data_in = card->resample.buffer + get_input_frames_to_byte(used_frames, channels);
		srcData.input_frames = frames - used_frames;
		srcData.data_out = data + get_input_frames_to_byte(resampled_frames, channels);
		medvdbg("data_in addr = 0x%x + %d\t", srcData.data_in, get_input_frames_to_byte(used_frames, channels));
...
audio_manager_result_t set_audio_stream_in(unsigned int channels, unsigned int sample_rate, int format)
{
	...
	ret = get_supported_sample_rate(INPUT);
	if (ret != AUDIO_MANAGER_SUCCESS) {
		return ret;
	}
	...
	card->resample.channel_ratio = channels / card->resample.channels;
	...
	if (sample_rate == config.rate) {
		...
		card->resample.buffer = malloc((int)((float)get_input_frames_to_byte(get_input_frame_count(), channels) / card->resample.ratio) + 1);	// +1 for floating point margin
...
audio_manager_result_t set_audio_stream_out(unsigned int channels, unsigned int sample_rate, int format)
{
	...
	ret = get_supported_sample_rate(OUTPUT);
	if (ret != AUDIO_MANAGER_SUCCESS) {
		return ret;
	}
...
int start_audio_stream_in(void *data, unsigned int frames)
{
	...
	do {
		...
		if (card->resample.necessary) {
			...
			card->resample.buffer_size = get_input_frames_to_byte(frames,channels);
...
unsigned int get_input_frames_to_byte(uint32_t frames, uint8_t channels)
{
	int buffer_size = 0;
	audio_card_info_t *card;
	if ((g_actual_audio_in_card_id < 0) || (frames == 0) || channels > AUDIO_STREAM_CHANNEL_STEREO) {
		return 0;
	}
	card = &g_audio_in_cards[g_actual_audio_in_card_id];
	buffer_size = frames * channels * pcm_format_to_bits(pcm_get_format(card->pcm)) >> 3;
	return buffer_size;
}
...
void print_audio_card_info(audio_io_direction_t direct)
{
	...
	get_supported_sample_rate(direct);
	for (j = 0; j < CONFIG_AUDIO_MAX_DEVICE_NUM; j++) {
